...
### Iterate N latest samples in the original dataset ###
...
### Iterate over N 'new' brightness adjusted data ###

# Initialize lists to store features for each image
average_brightness_list_newdata = []
predictions_list_newdata = [] 
targets_list_newdata = []
# Define the factor for brightness enhancement
brightness_factor = 5.0  # Adjust
...
for i in range(N):
    ...
    brightened_image_gray = Image.fromarray((brightened_image_np * 255).astype(np.uint8)).convert('L')

    # Calculate average brightness for the brightened image
    average_brightness = np.mean(brightened_image_gray)

    # Append to list
    average_brightness_list_newdata.append(average_brightness)
    
    predict = predictor.predict(train_dataset[i][0]).argmax()
    predictions_list_newdata.append(predict)
    
    targets = train_dataset[i][2]
    targets = targets.detach().cpu().numpy().argmax()
    targets_list_newdata.append(targets)
    
# Convert to tabular data, requirement by Evidently
data = {"avg_brightness": np.array(average_brightness_list_newdata), "target": np.array(targets_list_newdata), "prediction": np.array(predictions_list_newdata)} 
df_current_corrupted = pd.DataFrame(data) 
df_current_corrupted.loc[1:2,"avg_brightness"]=np.nan
